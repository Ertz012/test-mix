...
class ExperimentLogger:
    ...
    def _setup_traffic_logger(self):
        ...
        if os.stat(os.path.join(self.run_dir, f"{self.hostname}_traffic.csv")).st_size == 0:
            logger.info("timestamp,event_type,packet_id,message_id,src,dst,size,prev_hop,next_hop,flags")
    ...
    def log_traffic(self, event_type, packet, prev_hop="?", next_hop="?"):
        ...
        msg_id = getattr(packet, 'message_id', 'N/A')
        msg = f"{time.time()},{event_type},{packet.packet_id},{msg_id},{packet.src},{packet.destination},{len(packet.payload)},{prev_hop},{next_hop},{flags_str}"
